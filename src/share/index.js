...
class Share extends Component {
    ...
    return (
      <div className="share">
        ...
        >
          ...
          <span className="js_qrcode_close" title="关闭" onClick={this.handleCloseClick} />
          <div className="js_share_qrcode">
            <canvas width="120" height="120" style={{ display: 'none' }} id="canvas" />
            <img alt="Scan me!" className="js_qrcode_img" src={this.state.dataURL} />
          </div>
  ...
  handleShareLink(type) {
    ...
    shareUrl += 'url=' + encodeURIComponent(this.$data.link || document.location.href);
    shareUrl += '&desc=' + encodeURIComponent(this.$data.desc || '可以分享你的想法哦');
    shareUrl += '&title=' + encodeURIComponent(this.$data.title || document.title);
    shareUrl +=
      '&' +
      pic +
      '=' +
      encodeURIComponent(
        this.$data.pic || '//gw.alicdn.com/mt/TB110jzof6H8KJjy0FjXXaXepXa-299-105.png'
      );
